import { i18nProps } from '@/i18n';
import { useApp } from '@/store/app';

/* "Exam Already Taken" page. */
export default function AlreadyTakenPage() {
  const { showPage } = useApp();
  return (
    <>
      <nav className="bg-navy h-16 flex items-center">
        <div className="max-w-6xl mx-auto px-6 w-full">
          <span className="text-white font-extrabold text-lg">TEAM</span>
          <span className="text-white/70 text-xs ml-1.5 tracking-[0.2em]">UNIVERSITY</span>
        </div>
      </nav>
      <div className="max-w-xl mx-auto px-6 py-16 text-center">
        <div className="bg-white rounded-2xl shadow-lg p-8">
          <div className="w-16 h-16 rounded-full bg-red/10 flex items-center justify-center mx-auto mb-4">
            <svg className="w-8 h-8 text-red" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2"
                d="M12 15v2m-6 4h12a2 2 0 002-2v-6a2 2 0 00-2-2H6a2 2 0 00-2 2v6a2 2 0 002 2zm10-10V7a4 4 0 00-8 0v4h8z" />
            </svg>
          </div>
          <h1 className="text-2xl font-bold text-navy mb-2" {...i18nProps('taken_title')} />
          <p className="text-gray-600" {...i18nProps('taken_text')} />
          <button onClick={function () { showPage('landing-page'); }} className="btn-outline mt-6" {...i18nProps('btn_back_home')} />
        </div>
      </div>
    </>
  );
}

...
function hasAttemptedExam(n: string, p: string): boolean {
  const a = JSON.parse(
    localStorage.getItem("team_exam_attempts") || "[]",
  ) as Array<{ name: string; phone: string }>;
  for (let i = 0; i < a.length; i++)
    if (a[i].phone === p && a[i].name === n) return true;
  return false;
}
...
function saveStudentRecord(
  ...
  const r = JSON.parse(
    localStorage.getItem("team_exam_registrations") || "[]",
  ) as Array<Record<string, string>>;
  const i = arrFindIndex(r, function (x) {
    return x.phone === p;
  });
  const rec = {
    name: n,
...
export function AppProvider({ children }: { children: ReactNode }) {
  ...
  function showToast(message: string, type: ToastType = "info"): void {
    ...
    setToasts(function (prev) {
      return prev.concat([
        { id: id, message: message, type: type, leaving: false },
      ]);
    });
    setTimeout(function () {
      setToasts(function (prev) {
        return prev.map(function (x) {
          return x.id === id
            ? { id: x.id, message: x.message, type: x.type, leaving: true }
            : x;
        });
      });
      setTimeout(function () {
  ...
  function setLanguage(l: LangCode): void {
    ...
    setLang(l);
    try {
      localStorage.setItem("team_exam_lang", l);
    } catch (e) {
      /* ignore */
    }
    try {
  ...
  function handleRegistration(e: FormEvent<HTMLFormElement>): void {
    ...
    const p = (
      form.querySelector("#reg-phone") as HTMLInputElement
    ).value.trim();
    const em = (
    ...
    if (!n || !p) {
      showToast(t("toast_required"), "error");
      return;
    }
    if (!d) {
      showToast(t("toast_date"), "error");
      return;
    }
    if (!selectedExamType) {
      showToast(t("toast_type"), "error");
      return;
    }
    setStudentName(n);
    setStudentPhone(p);
    setStudentEmail(em || "");
    if (hasAttemptedExam(n, p)) {
      showPage("already-taken-page");
      return;
    }
  ...
  function showWaitingPage(d: string): void {
    ...
    const fd = arrFind(EXAM_DATES, function (x) {
      return x.date === d;
    });
  ...
  function toggleFlag(): void {
    const gi = getGlobalQuestionIndex(
      getCurrentSections(currentExam),
      currentSection,
      currentQuestion,
    );
  ...
  function selectWordChip(i: number): void {
    if (
      Object.keys(wordBankPlacements)
        .map(function (k) {
          return wordBankPlacements[Number(k)];
        })
        .indexOf(i) >= 0
    )
      return;
  ...
  function placeWordInGap(qi: number): void {
    if (selectedWord === null) {
      showToast(t("toast_word_first"), "info");
      return;
    }
  ...
  function toggleMobileSidebar(): void {
    setSidebarMobileOpen(function (v) {
      return !v;
    });
  }
  ...
  function closeTabWarning(): void {
    setTabWarning(function (prev) {
      return { visible: false, count: prev.count };
    });
  }
  ...
  function submitExam(): void {
    ...
    if (studentName && studentPhone)
      recordAttempt(studentName, studentPhone, currentExam);
    ...
    if (currentExam === "english") {
      ...
      for (let i = 0; i < grammarQs.length; i++) {
        total++;
        if (answers[i] === grammarQs[i].correct) {
          correct++;
          gCorrect++;
        }
      }
      ...
      for (let i = 0; i < wbQs.length; i++) {
        total++;
        if (wordBankPlacements[i] === (wb.correctMapping ?? [])[i]) {
          correct++;
          wbCorrect++;
        }
      }
      ...
      autoScore += rScore;
      sectionBreakdown +=
        "Reading: " + rCorrect + "/" + rTotal + " (" + rScore + " marks)\n";
      const saSec = sec[3];
      ...
      const wTask1WordCount = wTask1Text.trim()
        ? wTask1Text.trim().split(/\s+/).length
        : 0;
      const wTask2WordCount = wTask2Text.trim()
        ? wTask2Text.trim().split(/\s+/).length
        : 0;
      sectionBreakdown += "\n--- Writing Tasks (Manual Grading) ---\n";
      sectionBreakdown +=
        "Writing Task 1: " + wTask1WordCount + " words (15 marks max)\n";
      sectionBreakdown +=
        "Writing Task 2: " + wTask2WordCount + " words (25 marks max)\n";
      sectionBreakdown += "\nAuto-graded Score: " + autoScore + "/60\n";
      sectionBreakdown +=
        "Total Exam: 100 marks (60 auto-graded + 40 writing)\n";
    ...
    const scoreStr =
      autoScore + "/" + (currentExam === "english" ? "60" : mathTotal);
    ...
    createBitrixLead(
      ...
      studentEmail,
      examLabel,
      selectedExamDate || todayStr,
      function () {
        updateBitrixLeadWithResults(
          scoreStr,
          totalStr,
          sectionBreakdown,
          examLabel,
  ...
  function disqualifyStudent(): void {
    ...
    const examLabel = currentExam === "math" ? "Mathematics" : "English";
    const dqText =
      "Student was disqualified for leaving the exam page 3 times.";
    createBitrixLead(
      studentName,
      studentPhone,
      studentEmail,
      examLabel,
      selectedExamDate || new Date().toISOString().split("T")[0],
      function (leadId) {
        const updateUrl = BITRIX24_WEBHOOK_URL.replace(
          "crm.lead.add",
          "crm.lead.update",
        );
        ...
        fetch(updateUrl, {
          ...
          .then(function (d) {
            if (d.error) console.error("Bitrix DQ error:", d.error);
          })
          .catch(function (e) {
            console.error("Bitrix DQ network error:", e);
          });
      },
    );
  }
  ...
  function attachSecurityListeners(): void {
    ...
    document.addEventListener("contextmenu", function (e) {
      if (pageRef.current === "exam-interface") e.preventDefault();
    });
    ...
    document.addEventListener("visibilitychange", function () {
      ...
      if (tabSwitchCountRef.current >= 3) {
        disqualifyRef.current();
      } else {
        setTabWarning({ visible: true, count: tabSwitchCountRef.current });
      }
    });
    window.addEventListener("beforeunload", function (e) {
      if (pageRef.current === "exam-interface") {
        e.preventDefault();
        e.returnValue = "";
      }
    });
